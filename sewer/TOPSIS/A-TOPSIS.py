...
mau.delete(0, END); mau.insert(0, 'R2, MAE, MAPE, RMSE, RAE, RRSE')
sl_thongke = Entry()
def thongke_file():
    file_sl = filedialog.askopenfilenames(title="Open Statistical File(s)", filetype=(("csv files", "*.csv"),))
    if len(file_sl) > 0:
        chuoi = ''
...
btn_tk_file = Button(frame1, text="Open File(s)", font='Tahoma 9 bold', height=1, justify=CENTER, command=thongke_file)
...
btn_tk = Button(frame1, text="Modify", font='Tahoma 9 bold', height=1, justify=CENTER, command=thongke)
...
def topsis():
    file_sl = filedialog.askopenfilename(title="Open file to calculate A-TOPSIS algorithm", filetype=(("txt files", "*.txt"),))
    if len(file_sl) > 0:
                ...
                if column == len(gt):
                    ...
                    for n in range(len(gt)):
                        if gt[n].strip() == '1':
                            ...
                            messagebox.showerror('Criteria Error', str(gt[n]) + '\n' + 'Please re-check Criteria.'
                    ...
                    for n in range(2):
                        ...
                        tt = t-1; w.append(bien)
                    w, ww = np.array(w), []
                    d_positive, d_negative = pow((a_positive - matrix), 2), pow((a_negative - matrix), 2)
                    ...
                    for n in range(2):
                        ...
                        tt = t - 1
                    dd_positive, dd_negative = np.sqrt(np.array(dd_positive)), np.sqrt(np.array(dd_negative))
                    rank = []
                    ...
                    tong_cot_p_final = 1 + (1 / (np.log(matrix_p_final.shape[0]))) * tong_cot_p_final
                    ...
                    df = pd.DataFrame()
                    df['rank'], df['name'], df['ratio1'], df['ratio2'] = rank_final, name, rank[0], rank[1]
                    # Sap xep dataFrame theo thu tu giam dan
                    df_sort = df.sort_values(by=['rank'], ascending=False)
                    kq.write('Architecture'.center(15) + 'Ratio_Mean'.center(12) + 'Ratio_Std'.center(12) +
                             'Ratio_Total'.center(12) + 'Ranking'.center(9) + '\n')
                    name, ratio1 = df_sort['name'].values.tolist(), df_sort['ratio1'].values.tolist()
                    ratio2, rank = df_sort['ratio2'].values.tolist(), df_sort['rank'].values.tolist()
                    for n in range(len(name)):
                        kq.write(str(name[n]).center(15) + str(round(ratio1[n], 4)).center(12) +
                                 str(round(ratio2[n], 4)).center(12) + str(round(rank[n], 4)).center(12))
                        if str(rank[n]) == 'nan':
                            nn = '---'
                        else:
                            nn = n + 1
                    ...
                    messagebox.showerror('Inconsistent Data', 'Please re-check Criteria.' + '\n' +
                                         'Number of Criteria = Number of columns - 1.')
                chuoi.clear(); chuoi.append(dong)
            dem = 1
        fileopen.close(); kq.close()
        messagebox.showinfo(title="Phd Dissertation/Thesis", message="Calculation Complete.")
    else:
        messagebox.showerror('Error', 'Input File to calculate.')

btn_TOPSIS_open = Button(frame_TOPSIS, text="Run A-TOPSIS", font='Tahoma 9 bold', fg='white',
                         bg='blue', height=1, justify=CENTER, command=topsis)
